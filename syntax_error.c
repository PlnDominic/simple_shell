...
/**
 * repeated_characters - counts repeated characters
 *
 * @input: input string
 * @i: index
 * Return: repetitions
...
{
	if (*(input - 1) == *input)
		return (repeated_character(input - 1, j + 1));

	return (j);
}

/**
 * error_sep_op - This finds syntax errors in the code
 *
 * @input: input string
 * @i: index
 * @last: last char read
...
int error_sep_op(char *input, int i, char last)
{
	...
	if (*input == '|')
	{
		...
		if (last == '|')
		{
			count = repeated_character(input, 0);
			if (count == 0 || count > 1)
				return (i);
		}
	}
